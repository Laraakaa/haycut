...
mod tests {
    ...
    impl Drop for CurrentDirGuard {
        fn drop(&mut self) {
            env::set_current_dir(&self.original).expect("test cwd should be restored");
        }
    }

    fn enter_repo(root: &Path) -> CurrentDirGuard {
        static CWD_LOCK: OnceLock<Mutex<()>> = OnceLock::new();
        let lock = CWD_LOCK
            .get_or_init(|| Mutex::new(()))
            .lock()
            .expect("cwd lock should not be poisoned");
        let original = env::current_dir().expect("test cwd should be readable");
        env::set_current_dir(root).expect("test repo should be current directory");
        CurrentDirGuard {
            original,
            _lock: lock,
        }
