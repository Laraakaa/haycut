...
use crate::commands::agent::engine::{AgentEngine, AgentEvent, ControlCommand, MessageRole};
use crate::commands::agent::workflow::{ExecutorKind, Node, NodeOp, NodeStatus, Workflow};
use crate::commands::task::{self, TaskState};
use crate::store::{self, RUN_STORE_PATH};
use crossterm::event::{
...
pub(super) fn run(demo: bool) -> i32 {
    match ratatui::run(|terminal| -> io::Result<()> {
        ...
        let result = run_editor(terminal, demo);
...
fn run_editor(terminal: &mut ratatui::DefaultTerminal, demo: bool) -> io::Result<()> {
...
enum UiEvent {
    ...
    WorkflowSnapshot(Box<WorkflowInstance>),
    NodeStarted {
        workflow_id: usize,
        node_id: usize,
    },
...
fn context_snapshot_from_task(task: &TaskState, revision: usize) -> ContextSidebarSnapshot {
    let active_node = task
        .workflow
        .nodes
        .iter()
        .find(|node| node.status == NodeStatus::Running);
    let model_calls = task
        .route
        .iter()
        .filter(|entry| {
            matches!(
                entry.executor,
                ExecutorKind::WeakModel | ExecutorKind::StrongModel
            )
        })
        .count();
    let sources: Vec<_> = task
        .observations
        .iter()
        .map(|observation| ContextSource {
            reference_id: observation.id.clone(),
            category: observation.kind.clone(),
            tokens: observation.tokens.packet,
        })
        .collect();
    let model_usage = model_usage_for_task(task);
    let input_tokens = model_usage
        .iter()
        .map(|usage| usage_value_amount(usage.input))
        .sum();
    let output_tokens = model_usage
        .iter()
        .map(|usage| usage_value_amount(usage.output))
        .sum();
    let verification = task.verification_results.last().map_or_else(
        || "pending".to_string(),
        |result| {
            if result.passed {
                "passed".to_string()
            } else {
                format!("failed ({})", result.command.display())
            }
        },
    );
    ContextSidebarSnapshot {
        ...
        intent: task
            .intent
            .map(|intent| format!("{intent:?}").to_lowercase())
            .unwrap_or_else(|| "pending".into()),
        project: task.project.as_ref().map_or_else(
            || "pending".into(),
            |project| format!("{} · {}", project.language, project.test_command),
        ),
        packet_tokens: task.budget.packet_tokens_used,
        packet_soft_limit: task.budget.soft_tokens,
        packet_hard_limit: task.budget.hard_tokens,
        raw_tokens_avoided: task.budget.raw_tokens_avoided,
        selected_items: sources.len(),
        available_items: task.available_context.len(),
        compiled_context_tokens: sources.iter().map(|source| source.tokens).sum(),
        sources,
        model: active_node.map_or_else(|| "—".into(), |node| node.op.executor().name().to_string()),
        model_purpose: active_node.map_or_else(|| "—".into(), |node| node.op.name().to_string()),
        model_calls,
        input_tokens,
        cached_input_tokens: 0,
        output_tokens,
        active_node: active_node.map_or_else(|| "—".into(), |node| node.op.name().to_string()),
...
fn usage_value_amount(value: UsageValue) -> usize {
    match value {
        UsageValue::Unknown => 0,
...
fn workflow_spec() -> Workflow {
    ...
    .map(|(index, (id, dependencies, _label, outcome))| Node {
        ...
        produced_by: match id {
            "source" | "tests" => Some("context".to_string()),
            _ => None,
        },
...
fn reduce_events(events: &[UiEvent]) -> Vec<TimelineEntry> {
    ...
    for event in events {
        match event {
            ...
            UiEvent::WorkflowSnapshot(snapshot) => {
                    ...
                    .find(|entry| matches!(entry, TimelineEntry::Workflow(_)))
                {
                    *workflow = (**snapshot).clone();
                    ...
                    document.push(TimelineEntry::Workflow((**snapshot).clone()));
                }
            }
...
#[derive(Clone, Debug)]
pub(super) struct ModelTraceUsage {
    id: String,
    step_index: i64,
    purpose: String,
    input: UsageValue,
...
fn model_usage_for_task(task: &TaskState) -> Vec<ModelTraceUsage> {
    store::agent_traces_for_task(Path::new(RUN_STORE_PATH), &task.id)
        .unwrap_or_default()
        .into_iter()
        .filter_map(|trace| {
            let input = trace
                ...
                .map(UsageValue::Reported)
                .or_else(|| {
                    usize::try_from(trace.estimated_input_tokens)
                        .ok()
                        .map(UsageValue::Estimated)
                })?;
            let output = trace
                ...
                .map(UsageValue::Reported)
                .or_else(|| {
                    usize::try_from(trace.estimated_output_tokens)
                        .ok()
                        .map(UsageValue::Estimated)
                })?;
            Some(ModelTraceUsage {
                id: trace.id,
...
impl LiveSession {
    fn new() -> Self {
        ...
        let worker = thread::spawn({
            ...
            move || live_worker(command_rx, update_tx, stop_requested)
        });
        Self {
    ...
    fn send(&self, command: WorkerCommand) -> Result<(), String> {
        ...
        self.commands
            .as_ref()
            .ok_or_else(|| "The agent worker has already stopped".to_string())?
            .send(command)
            .map_err(|_| "The agent worker stopped unexpectedly".to_string())
    }
}

impl crate::commands::tui::runtime::AgentRuntime for LiveSession {
    fn send(&self, command: WorkerCommand) -> Result<(), String> {
        Self::send(self, command)
    }

    fn try_recv(&self) -> Result<WorkerUpdate, TryRecvError> {
    ...
    fn shutdown(&mut self) {
        ...
        // Closing the command channel lets an idle worker exit before join.
...
fn publish_live_command(
    ...
    engine
        .advance_streaming(task, command, |snapshot, event| {
            let checkpoint_error = matches!(&event, AgentEvent::NodeStarted { .. })
                .then(|| task::save_current(snapshot))
                .transpose()
                .err()
                .map(|error| error.to_string());
            let _ = updates.send(WorkerUpdate::Event {
                event,
                task: snapshot.clone(),
                usage: model_usage_for_task(snapshot),
            });
            if let Some(error) = checkpoint_error {
                let _ = updates.send(WorkerUpdate::Error(format!(
                    "Could not checkpoint workflow state: {error}"
                )));
                return false;
            }
    ...
    task::save_current(task).map_err(|error| error.to_string())
}
...
fn live_worker(
    ...
    while let Ok(command) = commands.recv() {
        let result = match command {
            WorkerCommand::Start(goal) => match task::start_current(goal, None) {
                Ok(task) => {
                    let _ = updates.send(WorkerUpdate::Snapshot(task.clone()));
                    current = Some(task);
                    publish_live_command(
                        &mut engine,
                        current.as_mut().expect("new task is present"),
            ...
            WorkerCommand::Reply(message) => current.as_mut().map_or_else(
                || Err("No active task to answer".to_string()),
                |task| {
                    publish_live_command(
                        &mut engine,
                        task,
            ...
            WorkerCommand::Approve => current.as_mut().map_or_else(
                || Err("No pending patch approval".to_string()),
                |task| {
                    publish_live_command(
                        &mut engine,
                        task,
            ...
            WorkerCommand::Reject(reason) => current.as_mut().map_or_else(
                || Err("No pending patch approval".to_string()),
                |task| {
                    publish_live_command(
                        &mut engine,
            ...
            WorkerCommand::Stop => current.as_mut().map_or(Ok(()), |task| {
        ...
        let _ = updates.send(WorkerUpdate::Idle);
    }
}
...
struct App {
    ...
    live_messages_seen: usize,
    live_trace_ids: HashSet<String>,
    live_active_model: Option<String>,
    live_task_open: bool,
...
impl App {
    fn new(demo: bool) -> Self {
        Self {
            ...
            workflow: None,
            mode: if demo {
                ExecutionMode::Demo(DemoDriver::default())
            } else {
                ExecutionMode::Live(Box::new(LiveSession::new()))
            },
            input_mode: InputMode::Prompt,
            live_messages_seen: 0,
            live_trace_ids: HashSet::new(),
            live_active_model: None,
            live_task_open: false,
...
impl App {
    fn is_demo(&self) -> bool {
        matches!(self.mode, ExecutionMode::Demo(_))
    }

    fn send_live(&mut self, command: WorkerCommand) -> bool {
        let ExecutionMode::Live(session) = &self.mode else {
            return false;
        };
    ...
    fn request_quit(&mut self) -> bool {
        if !self.is_demo() && (self.live_task_open || self.pending.is_some()) {
            self.quit_after_stop = true;
            return !self.send_live(WorkerCommand::Stop);
        }
        true
    }

    fn poll_live_updates(&mut self) -> bool {
        let update = match &self.mode {
            ExecutionMode::Demo(_) => return false,
            ExecutionMode::Live(session) => match session.try_recv() {
                Ok(update) => update,
                Err(TryRecvError::Empty) => return false,
                Err(TryRecvError::Disconnected) => {
                    WorkerUpdate::Error("The agent worker stopped unexpectedly".to_string())
                }
            },
        };
        match update {
            WorkerUpdate::Snapshot(task) => self.apply_live_snapshot(task),
            WorkerUpdate::Event { event, task, usage } => self.apply_live_event(event, task, usage),
            WorkerUpdate::Error(error) => {
                self.pending = None;
                if let Some(id) = self.live_active_model.take() {
                    self.append_event(UiEvent::ModelStream(ModelStreamEvent::CallFailed { id }));
                }
                self.append_event(UiEvent::AssistantResponse {
                    id: self.events.len(),
                    text: format!("Agent error: {error}"),
    ...
    fn reset_live_task(&mut self) {
        self.events.clear();
        self.timeline.clear();
        self.workflow = None;
        self.selected_node = None;
        self.context_snapshot = ContextSidebarSnapshot::default();
        self.live_messages_seen = 0;
        self.live_trace_ids.clear();
        self.live_active_model = None;
        self.live_task_open = true;
        ...
        match &event {
            AgentEvent::NodeStarted { node_id, op } => {
                if matches!(
                    ...
                    let id = format!("model-step-{}", task.route.len() + 1);
                    self.live_active_model = Some(id.clone());
                    self.append_event(UiEvent::ModelStream(ModelStreamEvent::CallStarted {
                        id,
                        purpose: format!("{} ({})", op.name(), op.executor().name()),
                        input: UsageValue::Unknown,
                    }));
                }
                if let Some(turn) = self.pending.as_mut() {
                    turn.activity = Some(format!("{} ({node_id})", op.name()));
                }
            }
            AgentEvent::Question(_) => {
                self.input_mode = InputMode::Answer;
            }
            AgentEvent::ApprovalRequired(approval) => {
                self.input_mode = InputMode::Approval;
                let preview = task.patch_text.as_deref().unwrap_or(&approval.summary);
                self.append_event(UiEvent::AssistantResponse {
                    id: self.events.len(),
                    text: format!("Approval required (y/n):\n{preview}"),
                });
            }
            AgentEvent::PatchProposed { summary } => {
                self.append_event(UiEvent::AssistantResponse {
                    id: self.events.len(),
                    text: format!("Patch proposed:\n{summary}"),
                })
            }
            AgentEvent::VerificationCompleted { summary } => {
                self.append_event(UiEvent::AssistantResponse {
                    id: self.events.len(),
                    text: format!("Verification: {summary}"),
                });
            }
            ...
            AgentEvent::Stopped(_) => {
                self.append_event(UiEvent::AssistantResponse {
                    id: self.events.len(),
                    text: "Agent stopped.".into(),
                });
                self.input_mode = InputMode::Prompt;
            }
            AgentEvent::Progress(summary) => {
                if let Some(turn) = self.pending.as_mut() {
                    turn.activity = Some(summary.clone());
                }
            }
            AgentEvent::ActionProposed(action) => {
                if let Some(turn) = self.pending.as_mut() {
                    turn.activity = Some(format!("Next action: {action:?}"));
                }
            }
        }
        self.sync_live_usage(usage);
        self.apply_live_snapshot(task);
    }

    fn sync_live_usage(&mut self, usage: Vec<ModelTraceUsage>) {
        for trace in usage {
            if !self.live_trace_ids.insert(trace.id) {
                continue;
            }
            let id = format!("model-step-{}", trace.step_index);
            ...
            if self.live_active_model.as_deref() == Some(id.as_str()) {
                self.live_active_model = None;
            }
            if let Some(turn) = self.pending.as_mut() {
                turn.activity = Some(trace.purpose);
            }
        }
    }

    fn apply_live_snapshot(&mut self, task: TaskState) {
        self.live_task_open = task.terminal_reason.is_none();
        for message in task.messages.iter().skip(self.live_messages_seen) {
        ...
        self.live_messages_seen = task.messages.len();
        self.append_event(UiEvent::WorkflowSnapshot(Box::new(WorkflowInstance {
            workflow: task.workflow.clone(),
        })));
        self.selected_node = task
            .workflow
            .nodes
            .iter()
            .position(|node| node.status == NodeStatus::Running)
            .or_else(|| {
                task.workflow
                    .nodes
                    .iter()
                    .rposition(|node| node.status != NodeStatus::Pending)
            });
        self.append_event(UiEvent::ContextSnapshot(Box::new(
            context_snapshot_from_task(&task, self.events.len()),
        )));
    }
    ...
    fn handle_event(&mut self, event: Event) -> bool {
        ...
        if !self.is_demo() && self.input_mode == InputMode::Approval {
            ...
            {
                self.input_mode = InputMode::Prompt;
                return self.send_live(WorkerCommand::Approve);
            }
            if key.code == KeyCode::Char('n')
                && !key
                    ...
                    .intersects(KeyModifiers::SHIFT | KeyModifiers::CONTROL | KeyModifiers::ALT)
            {
                self.input_mode = InputMode::Rejection;
                self.editor.reset();
                self.append_event(UiEvent::AssistantResponse {
                    id: self.events.len(),
                    text: "Enter an optional reason for rejecting the patch.".into(),
                });
                return true;
            }
            return false;
        ...
        if self.pending.is_some() && !self.is_demo() {
            return false;
        }
    ...
    fn submit(&mut self) -> bool {
        ...
        if prompt.trim().is_empty() && self.input_mode != InputMode::Rejection {
            return false;
        }
        if !self.is_demo() {
            return match self.input_mode {
                InputMode::Prompt => {
                    self.reset_live_task();
                    ...
                    self.editor.reset();
                    self.layout = LayoutMode::Chat;
                    self.send_live(WorkerCommand::Start(prompt))
                }
                InputMode::Answer => {
                    self.append_event(UiEvent::UserSubmitted {
                        id: self.events.len(),
                    ...
                    self.editor.reset();
                    self.input_mode = InputMode::Prompt;
                    self.send_live(WorkerCommand::Reply(prompt))
                }
                InputMode::Rejection => {
                    let reason = if prompt.trim().is_empty() {
                        "rejected in TUI".to_string()
                    } else {
                        prompt
                    };
                    self.append_event(UiEvent::UserSubmitted {
                        id: self.events.len(),
                        text: format!("Reject patch: {reason}"),
                    });
                    self.editor.reset();
                    self.input_mode = InputMode::Prompt;
                    self.send_live(WorkerCommand::Reject(reason))
                }
                InputMode::Approval => false,
            };
        }
        self.append_event(UiEvent::UserSubmitted {
            id: self.events.len(),
        ...
        let event = match &mut self.mode {
            ExecutionMode::Demo(demo) => demo.start(),
            ExecutionMode::Live(_) => return false,
        };
        self.apply_demo_event(event)
    }

    fn advance_demo(&mut self) -> bool {
        let ExecutionMode::Demo(demo) = &mut self.mode else {
            return false;
        };
        let Some(event) = demo.advance() else {
            return false;
        };
        let changed = self.apply_demo_event(event);
        let next_node = match &self.mode {
            ExecutionMode::Demo(demo) => demo.next_node,
            ExecutionMode::Live(_) => return false,
        };
        if changed && self.pending.is_none() && next_node < workflow_spec().nodes.len() {
            let event = match &self.mode {
                ExecutionMode::Demo(demo) => demo.start_next().expect("next demo node exists"),
                ExecutionMode::Live(_) => return false,
            };
            self.apply_demo_event(event)
        } else if changed && next_node >= workflow_spec().nodes.len() {
    ...
    fn tick(&mut self) -> bool {
        let mut changed = self.poll_live_updates();
...
fn workflow_display_parent(node: &ProjectedNode) -> Option<&str> {
    node.produced_by.as_deref()
}
...
fn render_workflow_children(
    ...
    let children: Vec<_> = visible
        .iter()
        .copied()
        .filter(|node| workflow_display_parent(node) == parent)
        .collect();
    for (index, node) in children.iter().enumerate() {
        ...
        if let Some(outcome) = node.outcome.as_deref().filter(|_| !compact) {
            lines.extend(
                ...
                .map(|text| {
                    Line::from(vec![
                        Span::styled(child_prefix.clone(), connector_style),
...
fn render_chat(
    ...
    if let Some(row) = cache.rows.iter_mut().find(|row| row.owner == "activity") {
        row.line = activity_footer_line_with_activity(
            meter,
            pending.and_then(|turn| turn.activity.as_deref()),
        );
    }
...
fn rendered_rows(
    ...
    rows.push(RenderedRow {
        ...
        owner: "activity".into(),
        local_row: 0,
        line: activity_footer_line_with_activity(
            meter,
            pending.and_then(|turn| turn.activity.as_deref()),
        ),
    });
...
fn workflow_graph_lines(
    ...
    for (index, node) in visible.iter().enumerate() {
        let mut depth = 0;
        let mut parent = node.produced_by.as_deref();
        while let Some(parent_id) = parent {
            ...
            parent = projection
                ...
                .and_then(|candidate| candidate.produced_by.as_deref());
        }
...
fn activity_footer_line_with_activity(
    meter: &ModelMeterSnapshot,
    activity: Option<&str>,
) -> Line<'static> {
...
mod tests {
    ...
    #[test]
    fn workflow_steps_stay_flat_and_outcomes_use_a_single_rail() {
        let mut workflow = workflow_spec();
    ...
    #[test]
    fn only_spawned_tool_work_is_nested() {
        ...
        let plan = projection
            ...
            .unwrap();
        assert_eq!(workflow_display_parent(source), Some("context"));
        assert_eq!(workflow_display_parent(plan), None);
    }
    ...
    #[test]
    fn live_workflow_snapshot_replaces_the_demo_projection() {
        let mut app = App::default();
        let mut workflow = workflow_spec();
        workflow.nodes[0].id = "n1".into();
        workflow.nodes[0].status = NodeStatus::Done;
        workflow.nodes[1].id = "n2".into();
        workflow.nodes[1].status = NodeStatus::Running;
        workflow.nodes[1].produced_by = None;
        app.append_event(UiEvent::WorkflowSnapshot(Box::new(WorkflowInstance {
            workflow: workflow.clone(),
        })));

        let rendered = app.workflow.as_ref().unwrap();
        assert_eq!(rendered.workflow.nodes[0].id, "n1");
        assert_eq!(rendered.workflow.nodes[1].status, NodeStatus::Running);
        assert_eq!(
            workflow_projection(&rendered.workflow).nodes.len(),
            workflow.nodes.len()
        );
    }
    ...
    #[test]
    fn live_trace_usage_completes_the_active_model_counter() {
        let mut app = App::default();
        app.append_event(UiEvent::ModelStream(ModelStreamEvent::CallStarted {
            id: "model-step-3".into(),
            purpose: "direct_answer (strong model)".into(),
            input: UsageValue::Unknown,
        }));
        app.live_active_model = Some("model-step-3".into());

        app.sync_live_usage(vec![ModelTraceUsage {
            id: "trace-3".into(),
            step_index: 3,
            purpose: "final_report".into(),
            input: UsageValue::Reported(1_234),
            output: UsageValue::Reported(56),
        }]);

        let meter = model_meter(&app.events);
        assert_eq!(meter.active, None);
        assert_eq!(meter.input_total, UsageValue::LowerBound(1_234));
        assert_eq!(meter.output_total, UsageValue::LowerBound(56));
        assert!(app.live_active_model.is_none());
    }
    ...
    fn activity_footer_shows_usage_without_a_second_spinner() {
        ...
        let line = activity_footer_line_with_activity(&meter, None).to_string();
